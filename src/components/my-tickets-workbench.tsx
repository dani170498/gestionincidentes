...
type WorkbenchMode = "standard" | "resolution-edit";
...
type WorkbenchConfig = {
  ...
  submitLabel: string;
  manageLabel: string;
  statusWhenResolved: string;
  allowResolutionEdit: boolean;
  allowManageResolved: boolean;
};

const WORKBENCH_CONFIG: Record<WorkbenchMode, WorkbenchConfig> = {
  standard: {
    heroKicker: "Gestión operativa",
    heroTitle: "Mis tickets asignados",
    heroCopy:
      "Atiende, documenta y cierra tickets desde un panel único. El detalle del ticket y los popups ahora separan claramente contexto, acción e historial para evitar pasos ambiguos.",
    trayTitle: "Prioriza por estado y entra al detalle cuando vayas a operar",
    trayCopy: "Cada fila resume el contexto mínimo para decidir si documentas, cierras o reasignas.",
    emptyCopy: "Cuando ingresen o cambien de estado, aparecerán aquí para que puedas gestionarlos.",
    historyCopy:
    ...
    resolutionHint:
      "La fecha y la hora de resolución se generan automáticamente cuando el ticket se guarda con estado RESUELTO.",
    submitLabel: "Guardar cambios",
    manageLabel: "Gestionar",
    statusWhenResolved: "Ticket cerrado",
    allowResolutionEdit: false,
    allowManageResolved: false,
  },
  "resolution-edit": {
    heroKicker: "Ajustes operativos",
    heroTitle: "Mis tickets modificaciones",
    heroCopy:
      "Replica la bandeja operativa habitual, pero con un formulario de gestión orientado a corregir cierre y trazabilidad sin salir del ticket.",
    trayTitle: "Ubica tickets y corrige resolución desde el mismo flujo operativo",
    trayCopy: "La bandeja conserva el contexto de atención y agrega un panel pensado para ajustes finos de cierre.",
    emptyCopy: "Cuando existan tickets asignados en este estado, podrás entrar para corregir su resolución.",
    historyCopy:
    ...
    allowResolutionEdit: true,
    allowManageResolved: true,
  },
};
...
export function MyTicketsWorkbench({ mode }: { mode: WorkbenchMode }) {
  const config = WORKBENCH_CONFIG[mode];
  ...
  const resultsKey = `${mode}:${activeTab}:${items.length}:${loading ? "loading" : "ready"}`;
  ...
  function openEdit(ticket: Ticket) {
    if (isResolvedTicket(ticket) && !config.allowManageResolved) {
  ...
  async function save() {
    if (!selected || !edit) return;
    if (config.allowResolutionEdit && edit.estado === "RESUELTO" && (!edit.fecha_respuesta || !edit.hora_respuesta)) {
      setError("Debes completar fecha y hora de resolución para guardar un ticket resuelto");
      return;
    }
    ...
    if (data?.item) {
      ...
      if (data.item.estado === "RESUELTO") {
        const successLabel = config.allowResolutionEdit ? "Resolución actualizada" : "Ticket resuelto";
        toast.success(
          `${successLabel}: #${selected.id} · ${formatDateTime(data.item.fecha_respuesta, data.item.hora_respuesta)}`
        );
      }
    }
    await fetchTickets();
      ...
      <section className="hero-panel hero-panel--compact">
        <div className="hero-panel__content">
          <div className="page-header">
            <span className="page-kicker">{config.heroKicker}</span>
            <h1 className="page-title">{config.heroTitle}</h1>
            <p className="page-copy">{config.heroCopy}</p>
          </div>
      ...
      <section className="card">
        <div className="toolbar ticket-toolbar">
          <div className="stack ticket-toolbar__copy">
            ...
            <div>
              <h2 className="section-title">{config.trayTitle}</h2>
              <p className="page-lead">{config.trayCopy}</p>
            </div>
          </div>
          ...
          >
            {visibleItems.length === 0 ? (
              <div className="empty-state">
                ...
                <p className="muted">{config.emptyCopy}</p>
              </div>
            ) : (
                      ...
                      <tr key={item.id}>
                        ...
                        <td>
                          <div className="table-actions">
                            {isResolvedTicket(item) && !config.allowManageResolved ? (
                              <span className="status-chip status-chip--success">{config.statusWhenResolved}</span>
                            ) : (
                              <>
                                ...
                                <button className="button" type="button" onClick={() => openEdit(item)}>
                                  {config.allowResolutionEdit ? <CalendarClock size={15} /> : <ClipboardList size={15} />}
                                  {config.manageLabel}
                                </button>
                              </>
                            )}
            ...
            >
              <div className="dialog-panel__body form">
                ...
                <section className="form-section">
                  <div className="form-section__header">
                    ...
                    <p className="form-section__copy">{config.resolutionCopy}</p>
                  </div>
                  <div className="split">
                ...
                <div className="split">
                  ...
                  <p className="muted">{config.resolutionHint}</p>
                </section>
                ...
                <section className="ticket-sidecar">
                  <div className="ticket-sidecar__copy">
                    ...
                    <p className="page-lead">{config.historyCopy}</p>
                  </div>
              ...
              <div className="dialog-panel__footer dialog-panel__footer--sticky">
                ...
                <button className="button" type="submit" disabled={saving}>
                  <Save size={15} />
                  {saving ? "Guardando..." : config.submitLabel}
                </button>
              </div>
            </form>
          </DialogContent>
